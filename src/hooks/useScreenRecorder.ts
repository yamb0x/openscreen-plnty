...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const togglePause = useCallback(() => {
		const screen = screenRecorder.current?.recorder;
		if (!screen) return;
		const webcam = webcamRecorder.current?.recorder;

		if (screen.state === "recording") {
			try {
				screen.pause();
			} catch {
				// Recorder may already be transitioning.
			}
			if (webcam?.state === "recording") {
				try {
					webcam.pause();
				} catch {
					// Recorder may already be transitioning.
			...
			setPaused(true);
		} else if (screen.state === "paused") {
			try {
				screen.resume();
			} catch {
				// Recorder may already be transitioning.
			}
			if (webcam?.state === "paused") {
				try {
					webcam.resume();
				} catch {
					// Recorder may already be transitioning.
